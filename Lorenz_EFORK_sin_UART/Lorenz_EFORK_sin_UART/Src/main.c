...
static void efork_step(void)
{
    ...
    static uint32_t float_to_dac12(long double val, long double minVal, long double maxVal)
    {
        ...
        if (fabsl(rango) < 1e-16L) rango = 1e-16L;

        long double s = (val - minVal) * (4095.0L / rango);
        if (s < 0.0L) s = 0.0L;
        if (s > 4095.0L) s = 4095.0L;

        return (uint32_t)(s + 0.5L); // redondeo simple
    }
...
int main(void)
{
  ...
  if (HAL_DAC_Start(&hdac, DAC_CHANNEL_2) != HAL_OK) Error_Handler();

  /* Arrancar TIM2 con interrupción (200 Hz) */
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM2)
  {
    ...
    HAL_DAC_SetValue(&hdac, DAC_CHANNEL_1, DAC_ALIGN_12B_R,
                     float_to_dac12(x, -20.0L, 20.0L));
    HAL_DAC_SetValue(&hdac, DAC_CHANNEL_2, DAC_ALIGN_12B_R,
                     float_to_dac12(y, -25.0L, 25.0L));
  }
}
